...
let TARGET_CLIENT_ID = "cmp8awxnz0000l4n9g9wzs8vy";
...
async function main() {
  ...
  if (!client) {
    throw new Error(`Client with slug "${TARGET_SLUG}" not found.`);
  }
  ...
  // --- 2.5 Query all active standard parameters for linking -----------------
  const standards = await prisma.standardAuditParameter.findMany({
    where: { isActive: true },
    select: { id: true, name: true },
  });
  const standardParamMap = new Map(standards.map((s) => [s.name, s.id]));
  ...
  for (let i = 0; i < BEETEL_PARAMS.length; i++) {
    ...
    const standardParameterId = standardParamMap.get(p.category) ?? null;
